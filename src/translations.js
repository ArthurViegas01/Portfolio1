export const translations = {
  pt: {
    ...
    caseStudy: {
      ...
      terraform: {
        heroDesc: "Infraestrutura AWS completa como codigo (IaC) com Terraform modular. VPC, EC2, RDS e S3 definidos em modulos reutilizaveis com estado remoto em S3 + DynamoDB. Pipeline CI/CD via GitHub Actions: terraform plan automatico em PRs e terraform apply no merge para main.",
        tabs: { architecture: "Arquitetura", tradeoffs: "Trade-offs", scalability: "Escalabilidade", security: "Seguranca & IAM", cicd: "Pipeline CI/CD" },
        problem: {
          title: "O Problema Tecnico",
          text: "Provisionar infraestrutura manualmente via console AWS e propenso a erros, nao reprodutivel e impossivel de auditar. O desafio era criar uma infra completamente definida em codigo, com modulos reutilizaveis, estado compartilhado entre deploys e um pipeline automatizado que validasse mudancas antes de aplicar — seguindo principios de least-privilege e zero hardcoded secrets.",
        },
        architecture: {
          title: "Arquitetura e Modulos",
          intro: "A infra e organizada em 4 modulos Terraform independentes que se comunicam via outputs. O estado e armazenado remotamente no S3 com locking via DynamoDB, garantindo consistencia em operacoes concorrentes.",
          steps: [
            { title: "Modulo networking/", desc: "Define VPC (10.0.0.0/16), subnet publica com Internet Gateway, subnet privada reservada e Security Groups com regras minimas de ingress/egress." },
            { title: "Modulo compute/", desc: "Provisiona EC2 t3.micro na subnet publica. nginx como reverse proxy e FastAPI (uvicorn) configurados via user_data bash script no boot. Acesso via SSM Session Manager — sem SSH direto." },
            { title: "Modulo database/", desc: "RDS PostgreSQL db.t3.micro na subnet privada, acessivel apenas pelo Security Group do EC2. Controlado pela variavel create_rds=false por padrao (custo zero em dev)." },
            { title: "Modulo storage/", desc: "S3 bucket com versionamento habilitado para assets estaticos e backups. Politica de bucket com acesso restrito por IAM." },
            { title: "Estado Remoto (Bootstrap)", desc: "Script de bootstrap cria o S3 bucket de estado e a tabela DynamoDB para locking em uma unica execucao inicial. Todos os deploys subsequentes usam esse backend." },
          ],
        },
        tradeoffs: {
          title: "Trade-offs de Design",
          intro: "Decisoes de arquitetura tomadas para equilibrar custo, seguranca e simplicidade em um projeto de portfolio.",
          items: [
            {
              decision: "Sem NAT Gateway",
              alternative: "NAT Gateway padrao",
              verdict: "Custo zero — EC2 na subnet publica com SG restrito",
              pros: ["Elimina ~$32/mes de custo fixo de NAT", "EC2 ainda acessivel via SSM sem IP publico exposto", "Security Group limita ingress a HTTP/HTTPS"],
              cons: ["Subnet publica (mitigada por SG + SSM)", "Nao ideal para workloads com multiplas instancias privadas"],
            },
            {
              decision: "Modulos separados por responsabilidade",
              alternative: "Arquivo main.tf monolitico",
              verdict: "Modularidade permite reuso entre ambientes (dev/prod)",
              pros: ["Cada modulo tem outputs tipados consumidos por outros", "Facil de adicionar staging/prod sem duplicar codigo", "Testabilidade isolada por modulo"],
              cons: ["Mais arquivos para navegar inicialmente", "Outputs explicitamente declarados (boilerplate necessario)"],
            },
            {
              decision: "GitHub Actions para CI/CD",
              alternative: "Terraform Cloud / Atlantis",
              verdict: "Zero custo adicional, integrado ao repositorio",
              pros: ["tf-plan posta o diff direto como comentario no PR", "Secrets gerenciados pelo GitHub (sem tool extra)", "Workflow visivel no repositorio publico como showcase"],
              cons: ["Nao tem UI de aprovacao visual como Terraform Cloud", "Sem historico de runs alem dos GitHub Actions logs"],
            },
          ],
        },
        scalability: {
          title: "Escalabilidade da Infra",
          intro: "A estrutura modular permite escalar tanto a infra quanto o time sem reescrever o codigo — novos ambientes e regioes sao adicionados via variaveis.",
          pillars: [
            { icon: "uil uil-layer-group", color: "blue",   title: "Multi-ambiente",      desc: "dev/staging/prod como workspaces Terraform ou diretorios separados. Mesmos modulos, variaveis diferentes." },
            { icon: "uil uil-globe",        color: "green",  title: "Multi-regiao",        desc: "Variavel aws_region parametrizavel. eu-west-1 escolhida para Ireland target — trocar para us-east-1 e uma linha." },
            { icon: "uil uil-sync",         color: "purple", title: "Estado Compartilhado", desc: "S3 backend com versionamento permite rollback de state. DynamoDB lock previne race conditions em times." },
            { icon: "uil uil-shield-check", color: "orange", title: "Least-Privilege IAM", desc: "Cada componente tem sua propria IAM role com politica minima. Instance Profile no EC2 sem access keys hardcoded." },
            { icon: "uil uil-code-branch",  color: "red",    title: "GitOps Flow",         desc: "Toda mudanca de infra passa por PR com plan automatico. Ninguem aplica manualmente — o historico fica no git." },
            { icon: "uil uil-dollar-sign",  color: "teal",   title: "Custo Controlado",    desc: "RDS desligado por padrao (create_rds=false). NAT Gateway eliminado. EC2 t3.micro dentro do free tier." },
          ],
          bottleneck: "Escalar para auto-scaling exigiria mover EC2 para Launch Template + ASG + ALB. A estrutura modular atual suporta essa migracao sem reescrever networking ou IAM.",
        },
        security: {
          title: "Seguranca & IAM",
          intro: "Principios de least-privilege e zero hardcoded credentials aplicados em toda a stack.",
          items: [
            { icon: "uil uil-shield-check",  title: "Least-Privilege IAM",    desc: "Instance Profile com politica customizada — apenas permissoes necessarias para SSM, S3 e CloudWatch. Sem AdministratorAccess.", tag: "IAM" },
            { icon: "uil uil-lock-alt",       title: "Zero Hardcoded Secrets", desc: "Credenciais AWS apenas via GitHub Secrets (CI) ou variaveis de ambiente locais. Nenhuma key no repositorio.", tag: "Secrets" },
            { icon: "uil uil-server",         title: "SSM Session Manager",    desc: "Acesso ao EC2 sem abrir porta 22. Elimina necessidade de bastion host e gerencia de chaves SSH.", tag: "SSM" },
            { icon: "uil uil-filter",         title: "Security Groups",        desc: "Ingress minimo: HTTP/HTTPS publico, SSH restrito ao IP do admin. RDS acessivel apenas pelo SG do EC2.", tag: "Network" },
            { icon: "uil uil-history",        title: "State com Auditoria",    desc: "S3 com versionamento registra cada mudanca de state. DynamoDB lock garante que apenas um apply rode por vez.", tag: "S3 + DynamoDB" },
            { icon: "uil uil-eye",            title: "Mudancas auditaveis",    desc: "Toda alteracao de infra via PR com terraform plan visivel. Historico completo no git — ninguem aplica manual.", tag: "GitOps" },
          ],
          perfTitle: "Metricas de Infra",
          metrics: [
            { value: "~4 min", label: "tempo medio de terraform apply (full stack)" },
            { value: "0",      label: "credenciais hardcoded no repositorio" },
            { value: "4",      label: "modulos Terraform independentes" },
            { value: "$0",     label: "custo de NAT Gateway eliminado" },
          ],
        },
        cicd: {
          title: "Pipeline CI/CD — Terraform",
          intro: "Dois workflows GitHub Actions: tf-plan valida cada PR postando o diff como comentario, tf-apply provisiona a infra no merge para main.",
          stages: [
            { title: "PR aberto → tf-plan.yml", desc: "Checkout do repositorio, setup do Terraform 1.7, terraform init com S3 backend e terraform plan. O output do plan e postado como comentario no PR — sem surpresas no merge." },
            { title: "Revisao do Plan",          desc: "O desenvolvedor revisa o diff de infra diretamente na interface do GitHub PR. Resources a criar/modificar/destruir ficam visiveis antes de qualquer mudanca real." },
            { title: "Merge para main → tf-apply.yml", desc: "Checkout, init e terraform apply com auto-approve. Credenciais AWS injetadas via GitHub Secrets (nunca no codigo). Apply falha explicitamente se o exit code nao for 0." },
            { title: "Estado Atualizado",        desc: "Terraform escreve o novo state no S3, libera o lock do DynamoDB e os recursos AWS ficam provisionados. Tags environment e project aplicadas em todos os recursos." },
          ],
          metricsTitle: "Metricas do Pipeline",
          metrics: [
            { value: "~2 min",  label: "tempo de tf-plan no PR" },
            { value: "~4 min",  label: "tempo de tf-apply completo" },
            { value: "0",       label: "aprovacoes manuais necessarias apos merge" },
            { value: "100%",    label: "mudancas de infra via PR (GitOps)" },
          ],
        },
        ctaGithub: "Ver no GitHub",
        ctaDemo: "README & Documentacao",
      },
    },
  },
  ...
  en: {
    ...
    caseStudy: {
      ...
      terraform: {
        heroDesc: "Complete AWS infrastructure as code (IaC) with modular Terraform. VPC, EC2, RDS and S3 defined in reusable modules with remote state in S3 + DynamoDB locking. CI/CD pipeline via GitHub Actions: automated terraform plan on PRs and terraform apply on merge to main.",
        tabs: { architecture: "Architecture", tradeoffs: "Trade-offs", scalability: "Scalability", security: "Security & IAM", cicd: "CI/CD Pipeline" },
        problem: {
          title: "The Technical Problem",
          text: "Provisioning infrastructure manually through the AWS console is error-prone, non-reproducible, and impossible to audit. The challenge was to create infrastructure fully defined as code, with reusable modules, shared state across deploys, and an automated pipeline that validates changes before applying — following least-privilege and zero hardcoded secrets principles.",
        },
        architecture: {
          title: "Architecture & Modules",
          intro: "The infrastructure is organized into 4 independent Terraform modules that communicate via outputs. State is stored remotely in S3 with DynamoDB locking, ensuring consistency across concurrent operations.",
          steps: [
            { title: "networking/ module", desc: "Defines VPC (10.0.0.0/16), public subnet with Internet Gateway, reserved private subnet, and Security Groups with minimal ingress/egress rules." },
            { title: "compute/ module",    desc: "Provisions EC2 t3.micro in the public subnet. nginx as reverse proxy and FastAPI (uvicorn) configured via a user_data bash script at boot. Access via SSM Session Manager — no direct SSH required." },
            { title: "database/ module",   desc: "RDS PostgreSQL db.t3.micro in the private subnet, accessible only from the EC2 Security Group. Controlled by create_rds=false by default (zero cost in dev)." },
            { title: "storage/ module",    desc: "S3 bucket with versioning enabled for static assets and backups. Bucket policy with IAM-restricted access." },
            { title: "Remote State (Bootstrap)", desc: "A bootstrap script creates the S3 state bucket and DynamoDB lock table in a single one-time run. All subsequent deploys use this backend." },
          ],
        },
        tradeoffs: {
          title: "Design Trade-offs",
          intro: "Architecture decisions made to balance cost, security, and simplicity in a portfolio project.",
          items: [
            {
              decision: "No NAT Gateway",
              alternative: "Standard NAT Gateway",
              verdict: "Zero cost — EC2 in public subnet with restrictive SG",
              pros: ["Eliminates ~$32/month fixed NAT cost", "EC2 still accessible via SSM without exposed public IP", "Security Group limits ingress to HTTP/HTTPS only"],
              cons: ["Public subnet placement (mitigated by SG + SSM)", "Not ideal for workloads with multiple private instances"],
            },
            {
              decision: "Responsibility-separated modules",
              alternative: "Single monolithic main.tf",
              verdict: "Modularity enables reuse across environments (dev/prod)",
              pros: ["Each module has typed outputs consumed by others", "Easy to add staging/prod without duplicating code", "Isolated testability per module"],
              cons: ["More files to navigate initially", "Outputs must be explicitly declared (necessary boilerplate)"],
            },
            {
              decision: "GitHub Actions for CI/CD",
              alternative: "Terraform Cloud / Atlantis",
              verdict: "Zero additional cost, integrated into the repository",
              pros: ["tf-plan posts the diff directly as a PR comment", "Secrets managed by GitHub (no extra tooling)", "Workflow visible in the public repository as a showcase"],
              cons: ["No visual approval UI like Terraform Cloud", "No run history beyond GitHub Actions logs"],
            },
          ],
        },
        scalability: {
          title: "Infrastructure Scalability",
          intro: "The modular structure allows scaling both the infrastructure and the team without rewriting code — new environments and regions are added via variables.",
          pillars: [
            { icon: "uil uil-layer-group", color: "blue",   title: "Multi-environment",    desc: "dev/staging/prod as Terraform workspaces or separate directories. Same modules, different variables." },
            { icon: "uil uil-globe",        color: "green",  title: "Multi-region",         desc: "Parameterized aws_region variable. eu-west-1 chosen for Ireland target — switching to us-east-1 is one line." },
            { icon: "uil uil-sync",         color: "purple", title: "Shared State",         desc: "Versioned S3 backend enables state rollback. DynamoDB lock prevents race conditions across teams." },
            { icon: "uil uil-shield-check", color: "orange", title: "Least-Privilege IAM",  desc: "Each component has its own IAM role with minimal policy. EC2 Instance Profile — no hardcoded access keys." },
            { icon: "uil uil-code-branch",  color: "red",    title: "GitOps Flow",          desc: "Every infrastructure change goes through a PR with automated plan. No one applies manually — full history in git." },
            { icon: "uil uil-dollar-sign",  color: "teal",   title: "Cost-Controlled",      desc: "RDS off by default (create_rds=false). NAT Gateway eliminated. EC2 t3.micro within the free tier." },
          ],
          bottleneck: "Scaling to auto-scaling would require moving EC2 to a Launch Template + ASG + ALB. The current modular structure supports that migration without rewriting networking or IAM.",
        },
        security: {
          title: "Security & IAM",
          intro: "Least-privilege and zero hardcoded credentials principles applied across the entire stack.",
          items: [
            { icon: "uil uil-shield-check",  title: "Least-Privilege IAM",    desc: "Custom Instance Profile policy — only the permissions required for SSM, S3, and CloudWatch. No AdministratorAccess.", tag: "IAM" },
            { icon: "uil uil-lock-alt",       title: "Zero Hardcoded Secrets", desc: "AWS credentials via GitHub Secrets (CI) or local env vars only. No keys in the repository, ever.", tag: "Secrets" },
            { icon: "uil uil-server",         title: "SSM Session Manager",    desc: "EC2 access without opening port 22. Eliminates the need for a bastion host and SSH key management.", tag: "SSM" },
            { icon: "uil uil-filter",         title: "Security Groups",        desc: "Minimal ingress: public HTTP/HTTPS, SSH restricted to admin IP. RDS accessible only from the EC2 Security Group.", tag: "Network" },
            { icon: "uil uil-history",        title: "Auditable State",        desc: "Versioned S3 records every state change. DynamoDB lock guarantees only one apply runs at a time.", tag: "S3 + DynamoDB" },
            { icon: "uil uil-eye",            title: "Auditable Changes",      desc: "Every infra change via PR with visible terraform plan. Complete history in git — no one applies manually.", tag: "GitOps" },
          ],
          perfTitle: "Infrastructure Metrics",
          metrics: [
            { value: "~4 min", label: "average terraform apply time (full stack)" },
            { value: "0",      label: "hardcoded credentials in the repository" },
            { value: "4",      label: "independent Terraform modules" },
            { value: "$0",     label: "NAT Gateway cost eliminated" },
          ],
        },
        cicd: {
          title: "CI/CD Pipeline — Terraform",
          intro: "Two GitHub Actions workflows: tf-plan validates every PR by posting the diff as a comment, tf-apply provisions the infrastructure on merge to main.",
          stages: [
            { title: "PR opened → tf-plan.yml",        desc: "Repository checkout, Terraform 1.7 setup, terraform init with S3 backend, and terraform plan. The plan output is posted as a PR comment — no surprises at merge time." },
            { title: "Plan Review",                     desc: "The developer reviews the infrastructure diff directly in the GitHub PR interface. Resources to create/modify/destroy are visible before any real change is made." },
            { title: "Merge to main → tf-apply.yml",   desc: "Checkout, init, and terraform apply with auto-approve. AWS credentials injected via GitHub Secrets (never in code). Apply fails explicitly if the exit code is non-zero." },
            { title: "State Updated",                   desc: "Terraform writes the new state to S3, releases the DynamoDB lock, and AWS resources are provisioned. environment and project tags applied to all resources." },
          ],
          metricsTitle: "Pipeline Metrics",
          metrics: [
            { value: "~2 min", label: "tf-plan time on PR" },
            { value: "~4 min", label: "full tf-apply time" },
            { value: "0",      label: "manual approvals needed after merge" },
            { value: "100%",   label: "infra changes via PR (GitOps)" },
          ],
        },
        ctaGithub: "View on GitHub",
        ctaDemo: "README & Docs",
